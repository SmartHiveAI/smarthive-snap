...
HOST_GW = '' # Gateway IP for smarthive-gw.local.
...
class mdnsListener:
    ...
    def add_service(self, zeroconf, type, name):
        ...
        logger.info("Service added: %s, info: %s, ip: %s" % (name, info, zeroconf.cache.entries_with_name('SmartHive-GW.local.')))
...
def start():
    ...
    desc = {"version": "0.1"}
    info = ServiceInfo("_http._tcp.local.", "SmartHive-CLC._http._tcp.local.", socket.inet_aton(ip), LOCAL_PORT, 0, 0, desc, LOCAL_HOST + ".")
    zeroconf = Zeroconf()
